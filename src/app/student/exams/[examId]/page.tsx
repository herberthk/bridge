...
export default async function StudentExamHistoryPage({ params }: { params: Promise<{ examId: string }> }) {
  ...
  const exam = { id: examSnap.id, ...examSnap.data()! } as any;
  ...
  const graded = attempts.filter((a: any) => a.score);
    ...
    <div className="flex flex-col gap-6">
      ...
      <Card>
        ...
        <CardContent className="flex flex-col gap-3">
          {[...attempts].reverse().map((a: any) => {
            const isRetake = !!a.retakeOf;
            const failed = a.answers?.filter((x: any) => x.graded?.correct === false).length ?? 0;
            const skipped = a.answers?.filter((x: any) => !x.response || x.response === "" || (Array.isArray(x.response) && x.response.length === 0)).length ?? 0;
            return (
              <div key={a.id} className={`rounded-xl border p-4 ${isRetake ? "bg-amber-500/5 border-amber-500/20" : "bg-card"}`}>
                <div className="flex flex-wrap items-center justify-between gap-2">
                  <div>
                    ...
                    <p className="text-muted-foreground text-xs">{a.submittedAt ? format(parseDate(a.submittedAt)!, "d MMM yyyy HH:mm") : a.createdAt ? format(parseDate(a.createdAt)!, "d MMM yyyy") : "—"} · {a.timeSpentSeconds ? `${Math.round(a.timeSpentSeconds/60)} min` : "—"}</p>
                  </div>
                ...
                {a.feedback?.improvements?.length > 0 && (
                  <p className="text-muted-foreground mt-2 text-xs"><span className="font-medium">Tip to 100%:</span> {a.feedback.improvements[0]}</p>
                )}
              </div>
            );
          })}
