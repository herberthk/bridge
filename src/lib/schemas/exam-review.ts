...
export const questionPatchSchema = z.object({
  ...
  options: z
    ...
    .nullable()
    .optional(),
  correctOptionIndex: z.number().int().min(0).max(25).nullable().optional(),
  correctBool: z.boolean().nullable().optional(),
  acceptableAnswers: z
  ...
  pairs: z
    .array(
      z.object({
        left: z.string().trim().max(200),
        right: z.string().trim().max(200),
      }),
    )
  ...
  workedExample: z.string().trim().max(4000).nullable().optional(),
  /**
