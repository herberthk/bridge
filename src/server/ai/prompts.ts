...
function mathsFormattingRules(level: SchoolLevel): string[] {
  return [
    ...
    "- Systems of linear equations MUST be formatted as a standalone display formula `$$...$$` with an `aligned` environment, including the opening:",
    "  correct: $$\\begin{aligned} x + 2y - z &= 4 \\\\ 2x + 5y + z &= 9 \\\\ x + y - 4z &= k \\end{aligned}$$",
    "  wrong:   $x + 2y - z &= 4 \\\\ 2x + 5y + z &= 9 \\\\ x + y - 4z &= k \\end{aligned}$$ (NEVER omit `$$\\begin{aligned}`)",
    "  Every line MUST have `&=` before the value, `\\\\` between lines, and one `\\begin{aligned}` for every `\\end{aligned}`.",
    ...
    "- `options`: each option is a complete, self-contained expression — `\"$\\\\frac{9}{5}$\"`, not `\"9/5\"` and not a fragment that only parses next to its neighbours. All four options must use the same form. Any LaTeX in an option MUST carry its own `$...$` delimiters: `\"$\\\\lambda_1 = 3$\"`, never bare `\"\\\\lambda_1 = 3\"` (bare commands reach students as literal backslashes).",
