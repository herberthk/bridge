...
function mathsFormattingRules(level: SchoolLevel): string[] {
  return [
    ...
    "- Every group must close: balanced `{}`, a `\\right` for every `\\left`, an `\\end{X}` for every `\\begin{X}`. An unclosed brace destroys the whole formula.",
    "- Piecewise and cumulative distribution functions MUST use a cases environment — never a bare brace group:",
    "  correct: $$f(x) = \\begin{cases} kx(2-x) & 0 \\le x \\le 2 \\\\ 0 & \\text{otherwise} \\end{cases}$$",
    "  wrong:   $f(x) = {kx(2-x), 0 \\le x \\le 2, 0, \\text{otherwise}$",
    "  Put `&` before each condition and `\\\\` between rows, and use `\\text{otherwise}` for words inside maths.",
